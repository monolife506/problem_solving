...
double rotatingCalipers(vector<vector2> &points)
{
    vector<vector2> hull;
    ...
    double ret = dist(hull[pl], hull[pr]);
    for (size_t i = 0; i <= n; ++i)
    {
        ...
        ret = max(ret, dist(hull[pl], hull[pr]));
    }
...
int main()
{
    ...
    vector<vector2> points(C);
    for (vector2 &p : points)
        cin >> p.x >> p.y;

    cout << fixed << setprecision(8) << rotatingCalipers(points) << '\n';
}